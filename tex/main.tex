...
This work improves PUGD as a computationally adaptive optimizer, with practical guidelines for perturbation scheduling. Code and results are available under \url{https://github.com/eeyzs1}.
\end{abstract}
\vspace{-1.25em}
...
\begin{figure}[htbp]
	...
	\caption{%todo
	Training histories of SGD and PUGD
...
According to \cite{foret2021sharpnessawareminimizationefficientlyimproving}, through a series of mathematical deductions and simplifications, the original SAM inequality can be updated to gradient descent with 
\begin{align*}
...
What PUGD did is set the $\rho = 1$, which is bound the perturbation radius in other words, though Foret et al. \cite{foret2021sharpnessawareminimizationefficientlyimproving} and \cite{kwon2021asamadaptivesharpnessawareminimization} inidicates in their articles that $\rho$ with different values can also generate competitive performance.
\begin{figure}[htbp]
	\center
	\includegraphics[width=\columnwidth]{images/SGD_LLC.pdf}
	\caption{%todo
	Loss landscape and contour of SGD
	}
	\label{fig:SGD_LLC}
\end{figure}
...
This motivates a two-way exploration into how perturbation radii (large vs. small) interact with training phases (early vs. late) — an underexplored dimension that may reveal optimal noise scheduling strategies.
Early training benefits from larger perturbation radii to escape poor initializations (e.g., saddle points in high-curvature regions), while later stages require smaller radii to fine-tune within flat minima basins. This aligns with the loss landscape's evolution: from chaotic high-loss terrain (needing exploration) to stable low-loss regions (needing exploitation). Adaptive decay balances both phases, leveraging noise for faster convergence and better generalization.

This inspires a mirrored examination between enlarged radii's early effects versus diminished radii's late-stage behaviors.
